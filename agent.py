...
# Add 'metrics' and 'turn_detector' imports from the old code
...
# Add 'turn_detector' from the old code
...
async def check_tasks_llm(transcript: str) -> list:
    """Use the LLM to determine which tasks are completed.
    
    Expects the LLM to return a string containing each task with a confidence score.
    """
    prompt = (
        f"Evaluate the following transcript fragment:\n\n"
        f"\"{transcript}\"\n\n"
        "For each of the following tasks, indicate if it is complete along with a confidence percentage:\n"
        "1. Introduce yourself in a professional setting\n"
        "2. Mention your reason for attending\n"
        "3. Politely wrap up the conversation\n\n"
        "Format your answer as:\n"
        "Task 1: yes (95%); Task 2: no (50%); Task 3: yes (92%)"
    )
    ...
    completed = []
    # Simple parsing: if the response contains "yes" with a number greater than 90, mark it complete.
    for task_num, task_name in zip(["Task 1", "Task 2", "Task 3"],
                                   ["Introduce yourself in a professional setting",
                                    "Mention your reason for attending",
                                    "Politely wrap up the conversation"]):
        match = re.search(f"{task_num}:\\s*yes\\s*\\((\\d+)%\\)", response_text, re.IGNORECASE)
...
async def entrypoint(ctx: JobContext):
    ...
    # We keep the same ElevenLabs TTS object from the new code
    ...
    # Add the turn_detector from the old code, along with min/max endpointing settings
    ...
    chunk_length_schedule=[80, 120, 200, 260],
        ...
        turn_detector=turn_detector.EOUModel(),  # from old code
        min_endpointing_delay=0.5,
        max_endpointing_delay=5.0,
        chat_ctx=initial_ctx,
    )

    # Add usage collector and metrics event handler from the old code
    ...
    lambda event: asyncio.create_task(
        handle_data_received(event.data, event.participant, event.kind, event.topic, assistant)
    ),
)


    # ✅ Define the async handler outside of room.on
    async def handle_data_received(payload: bytes, participant, kind, topic, assistant):
        message = payload.decode("utf-8")
        print(f"Received message: {message}")
        print(f"From participant: {participant.identity if participant else 'unknown'}")
        print(f"Kind: {kind}, Topic: {topic}")

        # If the topic is "agent-messages", handle as before:
        if topic == "agent-messages":
            asyncio.create_task(processTasksLLM(message, ctx.room))

async def processTasksLLM(message: str, room):
    """Process message using LLM and publish tasks in the background."""
    try:
        tasks_completed = await check_tasks_llm(message)  # Use check_tasks_llm()
        for task in tasks_completed:
            await room.local_participant.publish_data(f"Task completed: {task}".encode("utf-8"), topic="agent-messages")
    except Exception as e:
        print(f"Error in processTasksLLM: {e}")
